...
> + 确保客户端最后一次发送的确认报文能够到达服务器，因为这个报文可能会丢失。如果服务器没有收到确认报文，那么它会重新发送一次连接释放报文，客户端就能在2*MSL这个时间段内收到这个报文，接着给出回应报文，并且重启2MSL计时器。
> + 防止类似于三次握手中已失效的请求报文出现在本连接中。在这个时间段内，客户端可以使本连接持续时间内的所有报文从网络中消失。

###### 6.为什么关闭连接是四次挥手，建立连接确实三次握手？

> 建立连接时，ACK和SYN是一起发送的。
>
> TCP是全双工通信，释放连接时，仅仅表示客户端不再发送数据，但是可以接受数据，而服务端可以先发送一些数据，在发送连接释放报文，因此ACK和FIN是分开发送的，所以多了一次连接。

###### 7.如果已经建立了连接，客户端出现故障怎么办？

> TCP设有一个保活计时器，客户端如果出现故障，服务器不能一直等下去。服务器每收到客户端的请求都会重新复位这个计时器，时间通常设置为2小时，若2小时没有收到客户端的数据，服务器就会发送一个探测报文段，每隔75秒发送一次，若一连发送10个探测报文段后仍然没有响应，服务器就认为客户端出现了故障，就会断开连接。

###### 8.TCP和UDP的区别？

> + TCP是传输控制协议，提供面向连接、可靠的字节流服务，点对点通信，拥有流量控制和拥塞控制功能，开销大。
> + UDP是用户数据报协议，无连接，不可靠，尽最大努力交付，传输速度快，面向报文，支持一对一，一对多，多对一，多对多，开销小。
...
> + 滑动窗口：
>   + 发送端已经发送，接收端还是没有接收的；
>   + 发送端没有发送，接收端有空间接收这部分数据。
> + 零窗口期间，发送端停止发送数据。
> + 滑动窗口保证了顺序性，防止乱序。比如有1、2、3、4这4个报文段分组发送，第3个报文段丢失了，接受端没有收到第3个报文段，接收端将会发送ACK，确认收到1、2报文段，并将4报文段进行缓存，发送端会重传第3个报文段，接收端接受成功，则会将3、4报文段确认，若不成功，4报文段被丢弃。

###### 10.TCP/IP拥塞控制？

> + 拥塞控制：防止过多的数据注入到网络中，使得网络中的路由器或者链路不至于过载。拥塞控制的一个前提是网络能够承受现有的网络负荷，是一个全局性的过程。
> + 拥塞控制的4种方法：
>   + 慢开始：
>   + 拥塞避免：
>   + 快速重传：
